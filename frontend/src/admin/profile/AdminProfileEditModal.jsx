...
const AdminProfileEditModal = ({
  ...
  const [imagePreview, setImagePreview] = useState(() => {
    ...
    return `${BASE_IMAGE_URL}/api/${profile.image}`;
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response) {
        setErrorMessage(error.response.data?.detail || "Server validation error occurred.");
      } else if (error.request) {
        setErrorMessage("Network response error occurred.");
      } else {
        setErrorMessage("Could not update profile");
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl max-w-xl w-full p-6 shadow-2xl relative transform transition-all duration-300 scale-100 flex flex-col max-h-[90vh] ">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1  ">
          <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pr-1 space-y-4 max-h-[50vh] custom-scrollbar">
            ...
            {errorMessage && (
              <div className="text-red-500 font-semibold text-center text-md p-2 bg-red-50 dark:bg-red-950/30 rounded-xl">
                {errorMessage}
              </div>
            )}
            ...
            <div className="flex flex-col space-y-1.5">
              ...
              <div className="flex items-start space-x-3 border border-gray-300 dark:border-gray-700 px-3 py-2 rounded-xl focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 transition-all bg-transparent">
                <TextCursor size={30} className="text-gray-400 dark:text-gray-500 mt-2 shrink-0" />
                <textarea
                  id="bio"
                  value={bio}
                  onChange={(e) => setBio(e.target.value)}
                  placeholder="Tell us about yourself"
                  className="w-full bg-transparent text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none text-lg py-2 min-h-[100px] resize-none"
                />
              </div>
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800 flex justify-end space-x-3 shrink-0">
            <button
              type="button"
              onClick={() => setIsAdminProfileEditModal(false)}
              className="px-5 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 text-md font-semibold transition-colors cursor-pointer"
              disabled={loading}
            >
              Cancel
            </button>
