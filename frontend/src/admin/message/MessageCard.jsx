...
const MessageCard = ({ message }) => {
  ...
  const initialLetter = message?.sender_name
    ? message.sender_name[0]
    : "?";

  return (
    <div className='relative w-full max-w-5xl p-3 md:p-4 rounded-xl shadow-sm bg-gray-200/20 dark:bg-gray-700/20 font-body flex justify-between items-start gap-3 md:gap-5 overflow-hidden'>

      {/* Left side */}
      <div className='flex gap-3 md:gap-5 min-w-0 flex-1'>

        {/* Profile icon */}
        <div className='shrink-0 w-12 h-12 md:w-20 md:h-20 rounded-full flex items-center bg-gray-200 dark:bg-gray-700/60'>
          <span className="text-xl md:text-3xl font-bold text-gray-700 dark:text-gray-100 capitalize mx-auto">
            {initialLetter}
          </span>
        </div>

        {/* Message body */}
        <div className='flex flex-col gap-1 py-1 md:py-2 min-w-0 flex-1'>

          {/* Email */}
          <span className='font-semibold text-xs md:text-base text-gray-500/50 break-all'>
            {message?.sender_email}
          </span>

          {/* Username */}
          <span className='text-base md:text-xl font-semibold text-gray-700 dark:text-gray-100 truncate'>
            {message?.sender_name}
          </span>

          {/* Subject */}
          <span className='mt-1 md:mt-2 font-semibold text-sm md:text-base text-gray-700/50 dark:text-gray-500 break-words'>
            {message?.sender_subject}
          </span>

          {/* Message */}
          <p className='text-sm md:text-base text-gray-700/50 dark:text-gray-100/50 break-words leading-relaxed'>
      ...
      {/* Right side */}
      <div className='shrink-0 flex flex-col gap-2 items-end self-stretch justify-between py-1 md:py-2'>

        {/* Timestamp */}
        <span className='text-gray-500 dark:text-gray-400 font-semibold text-[10px] md:text-sm whitespace-nowrap'>
        ...
        <div className="relative group inline-block">
          ...
          <button
            onClick={() => handleDeleteMessage(message.id)}
            className="p-1.5 md:p-2 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors"
          >
            <Trash2 size={17} className="md:w-[18px] md:h-[18px]" />
          </button>

          {/* Tooltip */}
          <span className="absolute bottom-full right-0 md:left-1/2 md:-translate-x-1/2 mb-2 scale-0 group-hover:scale-100 transition-all duration-150 origin-bottom pointer-events-none whitespace-nowrap rounded-md bg-gray-900 px-2.5 py-1 text-xs font-medium text-white shadow-md dark:bg-gray-100 dark:text-gray-900">
            Delete Message

            <span className="absolute top-full right-2 md:left-1/2 md:-translate-x-1/2 border-4 border-transparent border-t-gray-900 dark:border-t-gray-100" />
          </span>
